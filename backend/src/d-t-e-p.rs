...
async fn get_realtime_from_redis(state: web::Data<AppState>) -> Result<HttpResponse, ApiError> {
    let mut conn = state.redis.lock().await
        .get_multiplexed_async_connection()
        .await
        .map_err(|e| ApiError::Redis(format!("Redis connection failed: {e}")))?;

    // Get all keys for last 60s (assuming keys are sensor:timestamp)
    let keys: Vec<String> = conn.keys("sensor:*").await.map_err(|e| ApiError::Redis(format!("Redis KEYS failed: {e}")))?;

    let mut data: Vec<EnhancedSensorData> = Vec::new();

    for key in keys {
        if let Ok(value) = conn.get::<_, String>(&key).await {
            if let Ok(sensor_data) = serde_json::from_str::<EnhancedSensorData>(&value) {
                data.push(sensor_data);
            }
        }
    }

    // Sort by timestamp descending and take last 60 entries
    data.sort_by(|a, b| b.data.timestamp.cmp(&a.data.timestamp));
    let last_60: Vec<_> = data.into_iter().take(60).collect();
...
async fn main() -> std::io::Result<()> {
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            .route("/api/realtime", web::get().to(get_realtime_from_redis))
    })
