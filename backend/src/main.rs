...
impl actix_web::ResponseError for ApiError {
    fn status_code(&self) -> actix_web::http::StatusCode {
        match self {
            ...
            ApiError::Redis(_) => actix_web::http::StatusCode::SERVICE_UNAVAILABLE,
            ApiError::MySql(_) => actix_web::http::StatusCode::SERVICE_UNAVAILABLE,
            ApiError::Io(_) => actix_web::http::StatusCode::INTERNAL_SERVER_ERROR,
            ApiError::Tcp(_) => actix_web::http::StatusCode::BAD_GATEWAY,
...
async fn read_sensor_from_tcp(host: &str, port: u16) -> Result<SensorData, ApiError> {
    let stream = TcpStream::connect((host, port)).await?;
    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    reader.read_line(&mut line).await?;
    let sensor = serde_json::from_str::<SensorData>(&line)
        .map_err(|_| ApiError::Validation("Failed to parse sensor JSON".to_string()))?;
    Ok(sensor)
}
    ...
    loop {
        ...
        let data = if state.config.use_serial {
            match read_sensor_from_tcp(&state.config.serial_tcp_host, state.config.serial_tcp_port).await {
                Ok(d) => d,
                Err(e) => {
                    warn!("TCP read failed, falling back to simulation: {:?}", e);
                    simulate_sensor_data()
                }
            }
        } else {
...
async fn get_realtime(state: web::Data<AppState>) -> Result<HttpResponse, ApiError> {
    let mem = state.memory.lock().await;
    let data: Vec<_> = mem.iter().rev().take(60).cloned().collect();
    Ok(HttpResponse::Ok().json(data))
}
